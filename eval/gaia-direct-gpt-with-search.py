#!/usr/bin/env python3
"""
GAIA Benchmark Evaluation Script with OpenAI GPT-5.1 + Web Search Tools
=======================================================================
...
Usage:
    uv run eval/gaia-direct-gpt-with-search.py --output-dir ./gaia_results
"""
...
async def run_single_query(
    ...
    """
    Run multi-turn inference with GPT-5.1 reasoning model using tool calls.
        ...
        for attempt in range(max_retries):
            try:
                ...
                if reasoning_content:
                    all_reasoning_content.append(f"[Turn {turn + 1}] {reasoning_content}")
                ...
                if message.tool_calls:
                    ...
                    combined_reasoning = "\n\n".join(all_reasoning_content) if all_reasoning_content else None
                    return (answer if answer else content, combined_reasoning)
    ...
    combined_reasoning = "\n\n".join(all_reasoning_content) if all_reasoning_content else None
    return (None, combined_reasoning)
...
def print_final_report(final_metrics: Dict, model_name: str) -> str:
    ...
    report_lines.append("📊 GAIA Benchmark Evaluation Report (GPT-5.1 + Web Search)")
    report_lines.append("=" * 80)
...
async def evaluate_gaia(
    ...
    print_green("=" * 80)
    print_green("GAIA Benchmark Evaluation (GPT-5.1 + Web Search)")
    print_green("=" * 80)
    ...
    checkpoint_file = output_dir / "checkpoint_gpt51_search.json"
    ...
    for idx, sample in enumerate(pbar):
        ...
        # Run inference
        prediction, reasoning_content = await run_single_query(
    ...
    results_file = output_dir / f"gaia_gpt51_search_results_{timestamp}.json"
    ...
    summary_file = output_dir / f"gaia_gpt51_search_summary_{timestamp}.json"
    summary_results = [
        {
            "task_id": r["task_id"],
            "level": r["level"],
    ...
    report_file = output_dir / f"gaia_gpt51_search_report_{timestamp}.txt"
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate GPT-5.1 on GAIA benchmark with web search tools")
    parser.add_argument(
        "--model",
        type=str,
        default="gpt-5.1",
